...
from .models import Match, Market, Selection, Sport
...
class MatchTests(APITestCase):
    def test_create_match(self):
        ...
        data = {
            ...
            "event": {
                ...
                "start_time": "2018-06-20 10:30:00",
                "sport": {
                    "id": 221,
                    "name": "Football"
                },
    ...
    def test_update_sport(self):
        ...
        response = self.client.post(url, create_match, format='json')
        # now we update the sport of the match
        update_name = {
            "id": 8661032861909884224,
            "message_type": "UpdateSportName",
            "event": {
                "id": 994839351740,
                "name": "Real Madrid vs Barcelona",
                "start_time": "2018-06-20 10:30:00",
        ...
        response = self.client.post(url, update_name, format='json')
        self.assertEqual(Sport.objects.get().name, 'Football')

    def test_update_selection(self):
        """
        Ensure we can update the selection of a market.
        """
        url = ('http://127.0.0.1:8000/api/match/')
        # first we create a match
        create_match = {
        ...
        response = self.client.post(url, create_selection, format='json')
        self.assertEqual(Match.objects.all()[0].market.selections.all()[0].id, 12345678910)
